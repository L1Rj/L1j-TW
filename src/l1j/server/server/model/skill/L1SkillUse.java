...
public class L1SkillUse {
	...
	public boolean checkUseSkill(L1PcInstance player, int skillid,
		...
		if (player instanceof L1PcInstance) {
			...
			int[] CheckSkillID = {
					2, 4, 5, 6, 7, 9, 10, 11, 14, 15, 16,
					17, 18, 20, 22, 23, 24, 25, 27, 28, 29, 30, 31, 32,
		...
		if (player instanceof L1PcInstance) {
			...
			if (jonje == null && player.getAccessLevel() != 200) {
				player.sendPackets(new S_SystemMessage("由於發現技能漏洞，即將強制中斷連線。"));
...
//change end
	...
	private void addMagicList(L1Character cha, boolean repetition) {
		...
		if (_skillId == CURSE_POISON) { // カーズポイズンの效果處理はL1Poisonに移讓。
			return;
		}
		...
		try {
			...
			for (Iterator<TargetStatus> iter = _targetList.iterator(); iter
				...
				if (_skill.getType() == L1Skills.TYPE_ATTACK
					...
					cha.removeSkillEffect(ERASE_MAGIC); // イレースマジック中なら、攻擊魔法で解除
				...
				if (cha.hasSkillEffect(_skillId) && _skillId != SHOCK_STUN) {
					...
					if (_skillId != SHAPE_CHANGE) { // シェイプ チェンジは變身を上書き出來るため例外
						continue;
					}
				}
...
//add end
				...
				// ★★★ 確率系スキル ★★★
					...
					if (cha instanceof L1PcInstance) {
						...
						|| _skillId == FREEZING_BLIZZARD) { // アイスランス、フリージングブリザード
