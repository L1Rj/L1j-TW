...
//    this.generateFKTriggers(sTableName);
  ...
  addTreeStyle: function(oStyle) {
    ...
    for (var iCss = 0; iCss < ss.length; iCss++) {
      if (ss[iCss].href == cssTreeDataTable) {
        bFound = true;
        break;
      }
    }
    ...
    var objDataTreeStyle = JSON.parse(oStyle);

    while (ss[iCss].cssRules.length > 0) {
      ss[iCss].deleteRule(0);
    }    
    ...
    for (var j in objDataTreeStyle) {
      ...
      for(var k = 0; k < objDataTreeStyle[j].length; k++) {
        if (objDataTreeStyle[j][k][0] == 'background-color') {
          if (objDataTreeStyle[j][k][2] != "")
            ruleSelCell += objDataTreeStyle[j][k][0] + ": " + objDataTreeStyle[j][k][2] + "; ";
          if (objDataTreeStyle[j][k][1] != "")
            ruleCell += objDataTreeStyle[j][k][0] + ": " + objDataTreeStyle[j][k][1] + "; ";
        }
        if (objDataTreeStyle[j][k][0] == 'color') {
          if (objDataTreeStyle[j][k][2] != "")
            ruleSelText += objDataTreeStyle[j][k][0] + ": " + objDataTreeStyle[j][k][2] + "; ";
          if (objDataTreeStyle[j][k][1] != "")
            ruleText += objDataTreeStyle[j][k][0] + ": " + objDataTreeStyle[j][k][1] + "; ";
        }
      }
      ruleSelCell += "}";
      ruleCell += "}";
      ...
      ss[iCss].insertRule(ruleSelCell, 0);
      ss[iCss].insertRule(ruleCell, 0);
      ss[iCss].insertRule(ruleSelText, 0);
      ss[iCss].insertRule(ruleText, 0);
    }
